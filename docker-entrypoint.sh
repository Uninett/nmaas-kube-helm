...
# Init helm if helm user is present
if [ -d /home/helm ]; then
    export HELM_HOME=/home/helm/.helm
    su -c "mkdir -p '$HELM_HOME'" helm
    if [ -d /home/helm/certs ]; then
	# If certs were mounted from outside, copy them, so we can keep a
	# writable $HELM_HOME
	cp /home/helm/certs/*.pem "$HELM_HOME/"
    fi
    su -c "helm init --client-only" helm
    cat >> /home/helm/.profile <<EOF
